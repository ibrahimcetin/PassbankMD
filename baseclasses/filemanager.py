...
class MDFileManager(ThemableBehavior, MDFloatLayout):
    ...
    def show(self, path):
        ...
        if self.preview:
            for name_dir in sorted(sorted(dirs, key=locale.strxfrm), key=str.casefold):
                manager_list.append(
                    {
            ...
            for name_file in sorted(sorted(files, key=locale.strxfrm), key=str.casefold):
                if (
            ...
            for name in sorted(sorted(dirs, key=locale.strxfrm), key=str.casefold):
            ...
            for name in sorted(sorted(files, key=locale.strxfrm), key=str.casefold):
    ...
    def back(self):
        ...
        if len(self.history) == 1:
            if self.history[0] == self.start_path:
                self.close()
                self.exit_manager(1)
                return
            self.history[0] = path
